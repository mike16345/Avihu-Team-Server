...
describe("JwtAuthService", () => {
  ...
  test("invalid expiration config", () => {
    ...
    const service = new JwtAuthService();
    expect(() => service.signAccessToken({ userId: "u1", role: "admin", sessionId: "s1" })).toThrow();
  });
  ...
  test("refresh token not found", async () => {
    ...
    await expect(service.validateRefreshToken("x")).rejects.toMatchObject({ statusCode: StatusCode.UNAUTHORIZED });
  });

  test("revoked refresh token", async () => {
    const service = new JwtAuthService() as any;
    service.sessionRepository = { findRefreshSessionByHash: jest.fn().mockResolvedValue({ userId: "u1", data: { revokedAt: new Date(), expiresAt: new Date(Date.now() + 10000) } }) };
    await expect(service.validateRefreshToken("x")).rejects.toMatchObject({ statusCode: StatusCode.UNAUTHORIZED });
  });

  test("expired refresh token", async () => {
    const service = new JwtAuthService() as any;
    service.sessionRepository = { findRefreshSessionByHash: jest.fn().mockResolvedValue({ userId: "u1", data: { revokedAt: null, expiresAt: new Date(Date.now() - 1000) } }) };
    await expect(service.validateRefreshToken("x")).rejects.toMatchObject({ statusCode: StatusCode.UNAUTHORIZED });
  });
  ...
  test("successful refresh session validation", async () => {
    ...
    const session = { _id: "s1", userId: "u1", data: { revokedAt: null, expiresAt: new Date(Date.now() + 1000) } };
